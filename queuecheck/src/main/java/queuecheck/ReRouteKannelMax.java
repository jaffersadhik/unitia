...
public class ReRouteKannelMax {
	...
	private static ReRouteKannelMax obj=null;


	private ReRouteKannelMax(){
	...
	public static ReRouteKannelMax getInstance(){
		
		
		if(obj==null){
			
			obj=new ReRouteKannelMax();
		}
	...
	private void checkQueueTableAvailable() {
		...
		try{
			...
			if(!table.isExsists(connection, "queue_max_reroute")){
				
				table.create(connection, "create table queue_max_reroute(queuename varchar(50),count numeric(10,0),updatetime numeric(13,0),mode varchar(25) default 'production')", false);
			}
	...
	public void doProcess() throws SQLException{
		...
		try{
			...
			Map<String,String> map=ReRouteKannelCount.getInstance().getMap();
	...
	private Map<String, String> getDLRCount(Connection connection) {
		...
		try{
			statement=connection.prepareStatement(getQuery("select queuename,count from queue_max_reroute",""));
...
private void insertQueueintoDB(Connection connection,String queuename,String count) {
		...
		
		{
			...
			insert=connection.prepareStatement("insert into queue_max_reroute(queuename,count ,updatetime,mode) values(?,?,?,?)");
	...
	private int updateDB(Connection connection,String queuename,String count) {
		...
		
		{
			...
			update=connection.prepareStatement("update queue_max_reroute set count=?,updatetime=? where queuename=? and mode=?");
