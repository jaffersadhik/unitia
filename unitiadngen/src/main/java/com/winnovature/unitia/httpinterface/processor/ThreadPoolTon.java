...
public class ThreadPoolTon {
	...
	private Map<String, Map<String, String>> getConfig() {
		...
		try{
			...
			connection=CoreDBConnection.getInstance().getConnection();
			statement =connection.prepareStatement("select poolname,poolsize,maxpoolsize,queuesize,keepalivetime,tablecount from workerpool where pooltype='dngen' ");
