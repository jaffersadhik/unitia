...
public class ReRouteKannelCount {
	...
	static String SQL="select username,count(*) cnt from reroute_kannel  group by username";
	
	private static ReRouteKannelCount obj=null;

	private  Map<String,String> map=new HashMap<String,String>();

	private ReRouteKannelCount(){
	...
	public static ReRouteKannelCount getInstance(){
		
		
		if(obj==null){
			
			obj=new ReRouteKannelCount();
		}
	...
	private void checkQueueTableAvailable() {
		...
		try{
			...
			if(!table.isExsists(connection, "queue_count_reroute")){
				
				table.create(connection, "create table queue_count_reroute(queuename varchar(50),count numeric(10,0),updatetime numeric(13,0),mode varchar(25) default 'production')", false);
			}
	...
	private Map<String, String> getEmptyMap(Connection connection) {
		...
		try{
			statement=connection.prepareStatement("select queuename from queue_count_reroute");
...
private void insertQueueintoDB(Connection connection,String queuename,String count) {
		...
		
		{
			...
			insert=connection.prepareStatement("insert into queue_count_reroute(queuename,count ,updatetime,mode) values(?,?,?,?)");
	...
	private int updateDB(Connection connection,String queuename,String count) {
		...
		
		{
			...
			update=connection.prepareStatement("update queue_count_reroute set count=?,updatetime=? where queuename=? and mode=?");
