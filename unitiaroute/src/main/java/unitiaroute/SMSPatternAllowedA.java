...
public class SMSPatternAllowedA {

	private static boolean isTableAvailable=false;

	private static SMSPatternAllowedA obj=null;
	
	private Map<String,List<Map<String,String>>> smspatternallowedset=new HashMap<String,List<Map<String,String>>>();

	
	private SMSPatternAllowedA(){
	
		reload();
	}
	
	public static SMSPatternAllowedA getInstance(){
		
		if(obj==null){
			
			obj=new SMSPatternAllowedA();
		}
	...
	public void reload(){
		...
		try{
			...
			statement =connection.prepareStatement("select pattern_id,senderid,smspattern from allowed_smspattern  where senderid is not null");
			resultset=statement.executeQuery();
			while(resultset.next()){
				
				String username=resultset.getString("senderid");
	...
	public List<Map<String,String>> getAllowedPaternSet(String senderid){
		
		return smspatternallowedset.get(senderid);
	}
