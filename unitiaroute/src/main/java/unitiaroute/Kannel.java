...
public class Kannel
{
	...
	private void loadSMSCID(Connection connection, TableExsists table) {
		...
		if(table.create(connection, SQLQuery.CREATE_SMSCID_TABLE, false)) {
			...
			table.insertKannel(connection,"apps","unitia.dngen.ip",9201,"4");
			table.insertKannel(connection,"reapps","unitia.dngen.ip",9201,"4");
			table.insertKannel(connection,"retry","unitia.dngen.ip",1111,"5");
