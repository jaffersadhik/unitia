...
public class DNGenProcessor
{
    ...
    public void handoverToDN(String DLRURL)
    {
        ...
        final Map<String,Object> reqmap = getRequestParam(url.getQuery());
        final Map drmap = getDR( reqmap.get("username").toString(),  reqmap.get("dnmsg").toString());
		if(ErrorCodeType.getInstance().isOTPRetry(reqmap.get("username").toString())){
			OTP_DN++;
